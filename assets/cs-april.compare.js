...
function compare_show(){    
  ...
  if(localStorage.getItem('cs-compare') != null ){
    ...
    $('.compare-page .compare-count').html(getID.length+' Gespeicherte Produkte');
    for(j=0; j<getID.length; j++){
      ...
      jQuery.getJSON(url, function(product) {
        ...
        $(wcn).find('.compare-addCart').append('<form action="/cart/add" method="post" class="variants" id="compare-form-cart-'+product.id+'" enctype="multipart/form-data"><div id="compare-variants-container-'+product.id+'" class="variants-wrapper"></div><div class="quantity-content"><label>translation missing: de.compare.qty</label><input type="text" size="5" class="item-quantity item-quantity-qs" name="quantity" value="1" /></div><div class="others-bottom"><a id="compare-addToCart-'+product.id+'" class="_btn btn-quick-shop add-to-cart">In den Einkaufswagen legen</a><div class="compare-remove" data-comparehandle="'+product.handle+'"><span class="lnr lnr-trash"></span></div></div></form>');  
        ...
        $(wcn).find('.compare-remove').on("click", function(){ 
          ...
          $('.compare-page .compare-count').html(storeID2.length+' Gespeicherte Produkte');
        });
